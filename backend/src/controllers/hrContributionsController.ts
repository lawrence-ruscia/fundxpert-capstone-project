...
export async function exportEmployeeContributionsPDFController(
  ...
  try {
    ...
    currentY += 15;
    doc.text(
      `Employment Status: ${employee.employment_status} | Salary: P${Number(employee.salary).toLocaleString()}`,
      60,
      currentY
    );
    ...
    doc
      ...
      .text('Total', columnPositions.total + 5, currentY, {
    ...
    contributions.forEach((contribution, index) => {
      ...
      doc
        ...
        .text(
          `P${Number(contribution.employee_amount).toLocaleString()}`,
          columnPositions.employeeAmount + 2,
          textY,
          { width: columnWidths.employeeAmount - 4, align: 'center' }
        )
        .text(
          `P${Number(contribution.employer_amount).toLocaleString()}`,
          columnPositions.employerAmount + 2,
          textY,
          { width: columnWidths.employerAmount - 4, align: 'center' }
        )
        .text(
          `P${contributionTotal.toLocaleString()}`,
    ...
    const summaryHeight = 150; // A safe height for the summary box and spacing

    // Check if the summary section fits on the current page, otherwise start a new one
    if (currentY + summaryHeight > doc.page.height - 80) {
      // 80 is a safe footer margin
      addFooter();
      doc.addPage();
      currentY = 40;
    }
