...
async function createRandomEmployee() {
  try {
    ...
    // Generate and insert contributions
    ...
    for (const contrib of contributions) {
      await pool.query(
        `INSERT INTO contributions 
         ...
         VALUES ($1, $2, $3, $4)`,
        [newEmployeeId, contrib.date, contrib.emp, contrib.er]
      );
    }
    ...
    if (Math.random() < 0.3) {
      ...
      await pool.query(
        `INSERT INTO loans (user_id, amount, status, repayment_term_months, purpose_category, consent_acknowledged, created_at)
         VALUES ($1, $2, $3, $4, $5, true, NOW() - INTERVAL '${Math.floor(Math.random() * 90)} days')`,
    ...
    if (Math.random() < 0.1 && contributions.length > 0) {
      ...
      await pool.query(
        `INSERT INTO withdrawal_requests (
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, true, NOW() - INTERVAL '${Math.floor(Math.random() * 30)} days')`,
        [
          newEmployeeId,
          withdrawalStatus,
          requestType,
