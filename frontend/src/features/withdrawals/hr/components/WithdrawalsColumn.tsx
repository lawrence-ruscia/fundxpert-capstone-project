...
export const withdrawalsColumns: ColumnDef<WithdrawalRequest>[] = [
...

  {
    ...
    cell: ({ row }) => (
      <div className='w-fit text-left font-medium text-nowrap'>
        {formatCurrency(row.getValue('payout_amount'))}
      </div>
    ),
    enableSorting: false,
  },
...

  {
    ...
    cell: ({ row }) => (
      <LongText className='max-w-36 ps-3'>
        {row.getValue('processed_at') !== null
          ? formatDisplayDate(new Date(row.getValue('processed_at')))
          : '-'}
      </LongText>
    ),
    filterFn: (row, id, value) => {
  ...
  {
    id: 'actions',
    cell: WithdrawalTableRowActions,
  },
];
